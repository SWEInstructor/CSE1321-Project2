...
public class Cave {
	public int cave(String NAME, String PLACE, String command, String[] inputs) {
			...
			while(!over) {
				...
				switch(command.toLowerCase()) {
					...
					case "use" :
						...
						String[] caveObjects = {"caveExit" , "cavePath"};
						String interactWith = Textgame.checkInput(caveObjects);
						
						if (interactWith .equals ("caveExit")) {
							System.out.print(NAME + " has decided to  make the smart choice, and leaves through the cave exit to go back to their home.");
							PLACE = "home";
						}
					
						else if (interactWith .equals ("cavePath")) {
							System.out.println("Unfortunately, " + NAME + " has decided to continue with their foolish quest, and venturee deep into the mysterious cave.");
							System.out.println("By sheer luck, " + NAME + " happens to stumble upon the location of the treasure, but as they take a step toward it,"
							...
							if (Inventory.hasItem("real map")) {
								...
								if (numberChoice == correctNumber) {
									...
									Inventory.addItemToInv("treasure");
								}
								else {
									//else{} is not in the pseudocode	
									System.out.print("Unfortunately, " + NAME + " has picked the wrong number and is not allowed to get the treasure! ");
								}
